...
@section('title', 'Hydra Joined Patients')
...
<style>
    ...
    /* Info Grid for details (optional if needed) */
    .info-grid {
        display: grid;
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <div>
            <h1 class="hospital-title" style="font-size: 1.5rem;">
                <i class="fas fa-user-check"></i> Hydra Joined Patients
            </h1>
            <p class="text-muted small mb-0">List of successfully joined Hydra patient records</p>
        </div>
        <div class="d-flex flex-wrap gap-2">
            ...
            <a href="{{ route('hydra.add.inquiry') }}" class="btn btn-primary-hosp">
    ...
    <div class="hospital-card p-2 mb-4">
        <form method="GET" action="{{ route('hydra.joined') }}" class="d-flex align-items-center gap-2">
            <div class="input-group" style="max-width: 400px;">
                <span class="input-group-text bg-white border-end-0 text-muted">
                    <i class="fas fa-search"></i>
                </span>
                <input type="text" name="search" class="form-control hospital-input border-start-0" 
                       placeholder="Search name, ID or city..." value="{{ request('search') }}" id="searchName" style="height: 38px;">
                @if(request('search'))
                <a href="{{ route('hydra.joined') }}" class="btn btn-outline-secondary border-start-0" title="Clear Search" style="height: 38px; display: flex; align-items: center;">
                ...
                <table class="table hospital-table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr data-id="{{ $inquiry->id }}">
                            <td>
                                <a href="{{ route('hydra.patient.profile', $inquiry->id) }}" class="text-decoration-none">
                                    <div class="profile-avatar">
                                        @if($inquiry->profile_image && Storage::disk('public')->exists($inquiry->profile_image))
                                            <img src="{{ asset('storage/' . $inquiry->profile_image) }}" alt="Profile Image" style="width:36px;height:36px;border-radius:50%;object-fit:cover;">
                                        @else
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-2">
                                    <a href="{{ route('hydra.patient.profile', $inquiry->id) }}" class="action-btn btn-profile" title="View Patient Details">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('hydra.edit', $inquiry->id) }}" class="action-btn btn-edit" title="Edit Patient">
                        ...
                        <tr>
                            <td colspan="8" class="text-center py-5">
                                <div class="py-4">
                                    ...
                                    <a href="{{ route('hydra.add.inquiry') }}" class="btn btn-primary-hosp mt-2">
                                        Add First Patient
                                    </a>
                                </div>
                            </td>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
            ...
            button.addEventListener('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        fetch('{{ route("hydra.destroy", ":id") }}'.replace(':id', id), {
                            method: 'POST',
                            body: formData,
                            headers: {
        ...
        document.getElementById('exportDataBtn').addEventListener('click', function() {
            ...
            window.location.href = '{{ route("hydra.export.joined") }}' + (search ? '?search=' + encodeURIComponent(search) : '');
        });

        document.getElementById('exportAllBtn').addEventListener('click', function() {
            window.location.href = '{{ route("hydra.export.all.joined") }}';
        });
    });
</script>
@endsection