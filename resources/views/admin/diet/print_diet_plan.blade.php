...
<div class="col-md-12 col-lg-10 m-auto p-0 print-container">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light bg-opacity-50">
                        <tr>
                            ...
                            <th class="text-muted text-uppercase small" style="letter-spacing: 1px; width: 30%;">Menu
                                Items</th>
                            <th class="text-muted text-uppercase small" style="letter-spacing: 1px; width: 25%;">
                                Nutrition</th>
                            ...
                            <th class="text-end pe-4 text-muted text-uppercase small"
                                style="letter-spacing: 1px; width: 25%;">Notes</th>
                        </tr>
                    </thead>
                    <tbody class="diet-body">
                        ...
                        <tr class="diet-row">
                            ...
                            <td>
                                <div class="fw-semibold text-dark mb-1">
                                    ...
                                    <div class="recipe-tags">
                                        ...
                                        <span class="badge bg-success bg-opacity-10 text-success fw-medium me-1 mb-1"
                                            style="font-size: 11px; border: 1px solid rgba(8, 104, 56, 0.1);">
                                            <i class="fas fa-leaf me-1 opacity-50"></i> {{ $recipe['name'] }}
                                            @if(($recipe['qty'] ?? 1) != 1 || !empty($recipe['unit']))
                                            <span class="ms-1 px-1 bg-success text-white rounded-pill"
                            ...
                            <td class="nutrition-cell">
                                ...
                                $menuQty = isset($menu['quantity']) && is_numeric($menu['quantity']) ? $menu['quantity']
                                : 1;
                                ...
                                <div class="nutrition-details" style="font-size: 10px; line-height: 1.4;">
                                    <div class="d-flex flex-wrap gap-1">
                                        <span class="text-dark"><span class="opacity-50">Cal:</span> {{
                                            round($totalCalories, 0) }}</span>
                                        <span class="text-dark"><span class="opacity-50">P:</span> {{
                                            round($totalProtein, 1) }}g</span>
                                        <span class="text-dark"><span class="opacity-50">C:</span> {{ round($totalCarbs,
                                            1) }}g</span>
                                        <span class="text-dark"><span class="opacity-50">F:</span> {{ round($totalFiber,
                                            1) }}g</span>
                                    </div>
                                </div>
                                @else
                        ...
                        <tr class="bg-light bg-opacity-50 fw-bold">
                            <td colspan="2" class="ps-4 py-3 text-uppercase small" style="color: #086838;">Daily Total
                                Nutrition</td>
                            <td class="py-3">
                                <div class="d-flex flex-wrap gap-2" style="font-size: 11px;">
                                    <span class="badge bg-success text-white">🔥 {{ round($dayTotalCalories, 0) }}
                                        kcal</span>
                                    <span class="badge bg-primary text-white">🥩 {{ round($dayTotalProtein, 1) }}g
                                        P</span>
                                    ...
                                    <span class="badge bg-secondary text-white">💧 {{ round($dayTotalFat, 1) }}g
                                        Fat</span>
                                </div>
                            </td>
                            <td colspan="2"></td>
                        ...
                        @if($dietPlan->diet || $dietPlan->exercise || $dietPlan->sleep || $dietPlan->water)
                        <tr class="bg-light bg-opacity-25">
                            <td colspan="5" class="ps-4 py-3">
                                <div class="d-flex flex-wrap gap-4" style="font-size: 12px;">
                                    @if($dietPlan->diet) <span><strong style="color: #086838;">Diet:</strong> {{
                                        $dietPlan->diet }}</span> @endif
                                    @if($dietPlan->exercise) <span><strong style="color: #086838;">Exercise:</strong> {{
                                        $dietPlan->exercise }}</span> @endif
                                    @if($dietPlan->sleep) <span><strong style="color: #086838;">Sleep:</strong> {{
                                        $dietPlan->sleep }}</span> @endif
                                    @if($dietPlan->water) <span><strong style="color: #086838;">Water:</strong> {{
                                        $dietPlan->water }}</span> @endif
                                </div>
                            </td>
                        </tr>
                        @endif
        ...
        <div class="card-footer border-0 py-4 px-4 d-flex justify-content-between align-items-center flex-wrap gap-3"
            ...
            <div>
                <span class="badge px-3 py-2 rounded-pill bg-info bg-opacity-10 text-info fw-bold">
                    <i class="fas fa-calendar-check me-2"></i>Next Follow-up: {{ date('d M, Y',
                    strtotime($dietPlan->next_follow_up_date)) }}
                </span>
            </div>
            @endif
    ...
    <div class="text-center mb-5 no-print">
        <a href="{{ route('patient.profile', ['id' => $dietPlan->patient_id]) }}"
